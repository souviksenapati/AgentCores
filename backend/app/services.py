...
class UserService:
    ...
    @staticmethod
    def update_user_role(db: Session, user_id: str, new_role: UserRole, updated_by: str) -> User:
        """Update user role"""
...
class SecurityService:
    ...
    def get_security_audit_data(db: Session, tenant_id: str) -> Dict[str, Any]:
        ...
        permission_violations = db.query(SecurityEvent).filter(
            SecurityEvent.tenant_id == tenant_id,
        ...
        risk_trends = db.query(
            ...
            SecurityEvent.tenant_id == tenant_id,
            SecurityEvent.created_at >= now - timedelta(days=7)
        ).group_by(func.date(SecurityEvent.created_at)).all()

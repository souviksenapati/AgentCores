...
from app.core.interfaces import AgentStatus as InterfaceAgentStatus
from app.core.interfaces import (
    AgentType,
    ProviderRegistry,
    ProviderType,
...
from app.models.database import AgentStatus, Task, TaskStatus, Tenant, TenantStatus
...
class AgentService(AgentServiceInterface):
    ...
    async def create_agent_from_config(
        self, config: AgentConfig, tenant_id: str, user_id: str
    ) -> str:
    ...
    async def create_agent(
        self, agent_data: AgentCreate, tenant_id: str, user_id: str = None
    ) -> Agent:
        ...
        try:
            ...
            agent_dict = {
                ...
                "instructions": getattr(
                    agent_data, "instructions", "You are a helpful AI assistant."
                ),
            ...
            logger.info(
                f"Enterprise agent {db_agent.agent_id} created for tenant {tenant_id}"
            )
            return db_agent
    ...
    async def execute_task(
        self, agent_id: str, task_request: TaskRequest, tenant_id: str
    ) -> TaskResult:
        ...
        try:
            ...
            provider_type = ProviderType(
                agent.model.split("/")[0] if "/" in agent.model else "openrouter"
            )
    ...
    async def get_agent(self, agent_id: str, tenant_id: str) -> Optional[Agent]:
        ...
        return (
            ...
            .filter(AgentModel.agent_id == agent_id, AgentModel.tenant_id == tenant_id)
            .first()
        )
    ...
    async def list_agents(self, tenant_id: str, user_id: str) -> List[Dict[str, Any]]:
        ...
        agents = (
            self.db.query(AgentModel).filter(AgentModel.tenant_id == tenant_id).all()
        )
        ...
        return [
            {
                ...
                "model": getattr(
                    agent, "model", "openrouter/meta-llama/llama-3.2-3b-instruct:free"
                ),
                "instructions": getattr(
    ...
    async def get_available_agents_for_connection(
        ...
        agents = (
            self.db.query(AgentModel)
            .filter(AgentModel.tenant_id == tenant_id, AgentModel.agent_id != agent_id)
            .all()
        )
    ...
    async def get_agent_analytics(
        self, agent_id: str, tenant_id: str, period: str = "24h"
    ) -> Dict[str, Any]:
...
class TaskService:
    ...
    async def get_task(self, task_id: str, tenant_id: str) -> Optional[Task]:
        ...
        return (
            self.db.query(Task)
            .filter(Task.task_id == task_id, Task.tenant_id == tenant_id)
            .first()
        )

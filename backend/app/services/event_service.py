...
class EventHandler:
    ...
    def __init__(self, handler_id: str, event_types: List[EventType], callback: Callable):
...
class EventService:
    ...
    def register_handler(
        ...
        try:
            ...
            logger.info(f"Registered event handler: {handler_id} for types: {[t.value for t in event_types]}")
            return True
...
class EventAnalytics:
    ...
    async def get_event_volume_by_type(
        self, 
        hours: int = 24
    ) -> Dict[str, int]:
    ...
    async def get_error_rate(self, hours: int = 24) -> float:
        ...
        for event_type in [EventType.TASK_FAILED, EventType.ENGINE_ERROR, EventType.PROVIDER_ERROR]:
            events = await self.event_service.get_events(
                event_type=event_type,
                since=since
            )
        ...
        for event_type in [EventType.TASK_SUBMITTED, EventType.TASK_COMPLETED, EventType.TASK_FAILED]:
            events = await self.event_service.get_events(
                event_type=event_type,
                since=since
            )

...
class TaskQueue:
    ...
    async def enqueue(self, task_definition: TaskDefinition):
        ...
        logger.info(f"Task enqueued: {task_definition.task_id} (priority: {task_definition.priority.value})")
    ...
    def get_task_status(self, task_id: str) -> Optional[TaskExecution]:
        ...
        for task_dict in [self._pending_tasks, self._running_tasks, self._completed_tasks]:
    ...
    def get_queue_stats(self) -> Dict[str, Any]:
        ...
        return {
            "pending": {priority.value: queue.qsize() for priority, queue in self._queues.items()},
...
class TaskExecutionEngine:
    ...
    async def start(self):
        ...
        logger.info(f"Task execution engine started with {self.max_concurrent_tasks} workers")
    ...
    async def submit_task(self, task_definition: TaskDefinition) -> str:
        ...
        try:
            ...
            if task_definition.depends_on:
                logger.info(f"Task {task_definition.task_id} has dependencies: {task_definition.depends_on}")
    ...
    async def _execute_task(self, task_execution: TaskExecution, worker_id: str):
        ...
        try:
            ...
            if not handler:
                raise ValueError(f"No handler registered for task type: {task_definition.task_type}")
            
            # Execute with timeout
            result = await asyncio.wait_for(
                handler(task_definition),
                timeout=timeout
            )
    ...
    async def _handle_task_error(self, task_execution: TaskExecution, error_msg: str, worker_id: str):
        ...
        logger.error(f"Task error: {task_id} - {error_msg} (attempt {task_execution.current_attempt})")
        
        # Check if we should retry
        if task_execution.current_attempt < task_definition.max_retries:
            logger.info(f"Retrying task: {task_id} (attempt {task_execution.current_attempt + 1})")
            ...
            await asyncio.sleep(min(2 ** task_execution.current_attempt, 30))  # Exponential backoff
            await self.task_queue.enqueue(task_definition)
    ...
    async def _handle_completion_task(self, task_definition: TaskDefinition) -> TaskResult:
    ...
    async def _handle_analysis_task(self, task_definition: TaskDefinition) -> TaskResult:

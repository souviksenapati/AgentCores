...
export const tenantAPI = {
  ...
  updateUser: (userId, userData) => {
    ...
    return api.put(
      ...
      sanitizeRequestData(userData)
    );
  },
  inviteUser: inviteData =>
    api.post('/invitations', sanitizeRequestData(inviteData)),
  getInvitations: () => api.get('/invitations'),
  acceptInvitation: acceptanceData =>
    api.post('/invitations/accept', sanitizeRequestData(acceptanceData)),
  deleteUser: userId => {
    if (!userId) {
...
export const agentAPI = {
  getAll: (params = {}) =>
    api.get('/agents', { params: sanitizeRequestData(params) }),
  getById: id => {
  ...
  getTasks: (id, params = {}) => {
    if (!id) return Promise.reject(new Error('Missing agent ID'));
    return api.get(`/agents/${encodeURIComponent(id)}/tasks`, {
      params: sanitizeRequestData(params),
    });
  },
  chat: (id, message) => {
    if (!id || !message)
      return Promise.reject(new Error('Missing agent ID or message'));
    return api.post(`/agents/${encodeURIComponent(id)}/chat`, {
      message,
      agent_id: id,
    });
  },
  getChatHistory: (id, limit = 50) => {
    if (!id) return Promise.reject(new Error('Missing agent ID'));
    return api.get(`/agents/${encodeURIComponent(id)}/chat/history`, {
      params: { limit },
    });
  },
...
export const taskAPI = {
  getAll: (params = {}) =>
    api.get('/tasks', { params: sanitizeRequestData(params) }),
  getById: id => {
...
export const securityAPI = {
  getAuditLogs: (params = {}) =>
    api.get('/security/audit-logs', { params: sanitizeRequestData(params) }),
  getSessionInfo: () => api.get('/security/session'),
  reportSecurity: incident =>
    api.post('/security/report', sanitizeRequestData(incident)),
  getSecuritySettings: () => api.get('/security/settings'),
  updateSecuritySettings: settings =>
    api.put('/security/settings', sanitizeRequestData(settings)),
};
...
export const secureRequest = {
  ...
  post: (url, data, config = {}) =>
    api.post(url, sanitizeRequestData(data), config),
  put: (url, data, config = {}) =>
    api.put(url, sanitizeRequestData(data), config),
  delete: (url, config = {}) => api.delete(url, config),
  patch: (url, data, config = {}) =>
    api.patch(url, sanitizeRequestData(data), config),
};

...
export default function SecurityAudit() {
  ...
  const securityTests = [
    {
      ...
      description:
        'Test if users can access protected routes without authentication',
      severity: 'critical',
    ...
    },
    {
      ...
      description:
        'Test if users can access resources beyond their role permissions',
      severity: 'high',
    ...
    try {
      ...
      if (!hasToken || !hasUser) {
        return {
          status: 'pass',
          message:
            'No authentication data found - user would be redirected to login',
  ...
  const testRoleEscalation = async () => {
    try {
      ...
      if (
        hasOwnerPermission === expectedOwnerAccess &&
        hasAdminPermission === expectedAdminAccess
      ) {
        return {
          status: 'pass',
  ...
  const testSessionSecurity = async () => {
    try {
      ...
      if (!sessionInfo) {
        ...
        if (sessionInfo.sessionDuration > 8 * 60 * 60 * 1000) {
          // 8 hours
        ...
        // Check idle timeout
        if (sessionInfo.lastActivity > 30 * 60 * 1000) {
          // 30 minutes
      ...
      if (!tokenExpiry) {
        issues.push(
          'Token expiry time not stored - could lead to stale sessions'
        );
      }
      ...
      if (issues.length === 0) {
        return {
          ...
          details:
            'Session timeout, token expiry, and idle detection working correctly',
        };
      } else {
        return {
          status: 'fail',
    ...
    try {
      ...
      if (sanitizedInput === xssTestString) {
        ...
        return {
          status: 'warning',
          message:
            'Basic input length limiting in place, but XSS protection may be insufficient',
    ...
    try {
      ...
      if (!hasRequestId) {
        return {
          ...
          details:
            'Request ID generation helps with CSRF protection and monitoring',
        };
      }
  ...
  if (
    !hasUserPermission('VIEW_SECURITY') &&
    !hasUserPermission('MANAGE_SECURITY')
  ) {
    return (
      <Box sx={{ p: 3 }}>
            ...
            <Card>
              <CardContent>
                ...
                <Typography color="text.secondary">
                  out of {auditResults.totalTests}
                </Typography>
              </CardContent>
            </Card>
          </Grid>
            ...
            <Card>
              <CardContent>
                ...
                <Typography variant="body1">
                  {auditResults.timestamp.toLocaleString()}
                </Typography>

...
const ProtectedRoute = ({
    ...
    const verifyAuthentication = async () => {
      ...
      try {
        ...
        if (requireRole && user.role !== requireRole) {
          console.warn(
            `Access denied. Required role: ${requireRole}, user role: ${user.role}`
          );
          setAuthCheck({
            checking: false,
            verified: false,
        ...
        if (requirePermission && !hasPermission(user.role, requirePermission)) {
          console.warn(
            `Access denied. Required permission: ${requirePermission}, user role: ${user.role}`
          );
          setAuthCheck({
            checking: false,
            verified: false,
  ...
  if (!authCheck.verified && authCheck.error === 'Authentication required') {
    console.log(
      'ProtectedRoute: Authentication required, redirecting to login'
    );
    return (
      <Navigate
        to={fallbackPath}
